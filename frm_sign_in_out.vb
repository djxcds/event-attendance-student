...
Public Class frm_sign_in_out
    ...
    Private Sub BunifuFlatButton1_Click(sender As Object, e As EventArgs) Handles btn_attendance_signer.Click
        Dim current_time As String = Format(global_functions.getCurrentDate, "HH:mm:ss")

        sessionChecker()
        If isEventActive Then
            If g_event_status = "SIGN IN" Then
                connection.conn.Open()
                query = "Update tbl_attendance SET community_labor_minutes = '" & penalty_half & "', log_in_time = '" &
                ...
                MsgBox("Hello " & student_first_name & "! You signed IN successfully!", MsgBoxStyle.Information)
                tb_user_query.Text = ""
                student_first_name = ""
                attendance_id = ""
                tb_student_name.Text = "Name of Student"

            ElseIf g_event_status = "SIGN OUT" Then
                If isStudentSignedIn() Then
                    query = "Update tbl_attendance SET community_labor_minutes = '0', log_out_time = '" &
                        current_time & "' WHERE event_entry_id='" & event_entry_id & "' and attendance_entry = '" &
                        attendance_id & "'"

                    message = "Hello " & student_first_name & "! You signed OUT successfully!"
                Else
                    query = "Update tbl_attendance SET community_labor_minutes = '" & penalty_half & "', log_out_time = '" &
                        current_time & "' WHERE event_entry_id='" & event_entry_id & "' and attendance_entry = '" &
                        attendance_id & "'"

                    message = "Hello " & student_first_name & "! It appears that you didn't sign in earlier, a penalty is added " &
                           "to you. You signed OUT successfully!"
                End If
                connection.conn.Open()
                cmd = New MySqlCommand(query, connection.conn)
                cmd.ExecuteNonQuery()
    ...
    Private Sub frmsigninout_Load(sender As Object, e As EventArgs) Handles Me.Load
        If g_event_status = "SIGN IN" Then
            btn_attendance_signer.Text = "SIGN IN"
        Else
            btn_attendance_signer.Text = "SIGN OUT"
        End If
    End Sub
    ...
    Private Sub tb_user_query_TextChanged(sender As Object, e As EventArgs) Handles tb_user_query.TextChanged
        Try
            ...
            If reader.Read Then
                ...
                btn_attendance_signer.Enabled = True
            Else
                tb_student_name.Text = "No student found in our records."
                student_first_name = ""
                attendance_id = ""
                btn_attendance_signer.Enabled = False
            End If
            connection.conn.Close()
